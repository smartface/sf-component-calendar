...
export type SpecialDaysBundleElement = {
  className?: string;
  day: number;
...
export type SpecialDaysService = {
  ...
  getSpecialDay(props: { year: number; month: number; day: number; calendar: string; lang: string }): { text: string; className: string }[];
...
function createSpecialDaysService(specialDays: SpecialDaysData): SpecialDaysService {
  ...
  return {
    ...
    getSpecialDay({ year = 0, month, day, calendar, lang }) {
      ...
      const keyByYearandAllCalendars = getKey({
        year,
        month,
        day,
        calendar: '*'
      ...
      const selectedDay = selectedDays.filter((aday) => {
        return !aday.langs.some((ln) => ln === '~' + lang) || aday.langs.some((ln) => ln === '*') || aday.langs.some((ln) => ln === lang);
      });

      return selectedDay.map((day) => ({
        text: day.text[lang] || day.text['*'],
  ...
  byYears.forEach((year) => {
    year.months.forEach((month) => {
      month.days.forEach((day) => {
        const newday: SpecialDaysBundleElement = {
          day: NaN,
          text: {},
          langs: []
        };
        ...
        Object.keys(day.calendars).forEach((calendar) => {
          ...
          acc[key] = acc[key] || [];
          if (day.className) newday.className = day.className;
