...
class Calendar extends CalendarDesign {
  ...
  constructor(private options?: CalendarOptions | undefined) {
    ...
    const { useRangeSelection = true, theme = null, justCurrentDays = false, calendarCore = null, useContext = true, useDaySelection = true } = this.options || {};
    ...
    this.navbar.onNext = this.nextMonth.bind(this);
    this.navbar.onPrev = this.prevMonth.bind(this);

    this._weeks.push(this.body.children.week1);
    this._weeks.push(this.body.children.week2);
    this._weeks.push(this.body.children.week3);
    this._weeks.push(this.body.children.week4);
    this._weeks.push(this.body.children.week5);
    this._weeks.push(this.body.children.week6);
  ...
  private _onSelectRange(weekIndex, weekDayIndex) {
    ...
    if (state.rangeSelectionMode === 0) {
      this.onRangeSelectionStart && this.onRangeSelectionStart(Object.assign({}, state.rangeSelection.start));
    } else if (state.rangeSelectionMode === 1) {
      this.onRangeSelectionComplete && this.onRangeSelectionComplete(Object.assign({}, state.rangeSelection.start), Object.assign({}, state.rangeSelection.end));
      typeof this.onDaySelect === 'function' && this.onDaySelect(this._calendarCore.getState().selectedDays || []);
    }
  }
  ...
  private doUpdateCalendar(oldState: CalendarState, newState: CalendarState) {
    if ((oldState.rangeSelectionMode === -1 && newState.rangeSelectionMode === 0) || (oldState.rangeSelectionMode === 1 && newState.rangeSelectionMode === -1)) {
      this.dispatch({
        //@ts-ignore
        type: 'deselectDays'
      });
    ...
    newState.month.daysMin.forEach((day, index) => {
      this.calendarDays.children['dayName_' + index].text = day;
    });

    (this.body as CalendarBody).setTomonth(newState.month.tomonth);
    (this.navbar as CalendarNavBar).setTomonth(newState.month.tomonth);
    // if(oldState.selectedDays.length > 0 && newState.selectedDays.length === 0){
    //     this.onDaySelect([]);
    // }

    this._weekMode ? this.setWeekMode(this._weekMode) : this.body.applyLayout();
  ...
  private _selectDay({ weekIndex, weekDayIndex }: { weekIndex: number; weekDayIndex: number }) {
    weekIndex >= 0 && weekDayIndex != null && this._weeks[weekIndex].setSelectedIndex(weekDayIndex);
  }
  ...
  _selectDayasRange({ weekIndex, weekDayIndexes, weekDayIndex }) {
    if (this._weeks[weekIndex] === undefined) throw new TypeError(`${weekIndex} Week cannot be undefined`);
    this._weeks[weekIndex].setRangeIndex(weekDayIndexes ? weekDayIndexes : weekDayIndex ? [weekDayIndex] : []);
  }
   ...
   */
  nextMonth() {
    if (this.onBeforeMonthChange && this.onBeforeMonthChange(this.currentMonth.nextMonth.normalizedDate) === false) {
      return;
    } else if (this.shouldMonthChange && this.shouldMonthChange(this.currentMonth.nextMonth.normalizedDate) === false) {
      return;
    }
   ...
   */
  prevMonth() {
    if (this.onBeforeMonthChange && this.onBeforeMonthChange(this.currentMonth.previousMonth.normalizedDate) === false) {
      return;
    } else if (this.shouldMonthChange && this.shouldMonthChange(this.currentMonth.nextMonth.normalizedDate) === false) {
      return;
    }
