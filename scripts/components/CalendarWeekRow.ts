...
export default class CalendarWeekRow extends CalendarWeekRowDesign {
  ...
  activateRangeSelection() {
    ...
    this._days.forEach((day, index) => (this.children[day].onTouch = () => this.onMove && this.onMove(index)));
  }

  deactivateRangeSelection() {
    this.isRangeSelection = false;
    this._days.forEach((day, index) => (this.children[day].onTouch = null));
  }
  ...
  init() {
    this._days.forEach((day, index) => addDaySelectEvent.call(this, this.children[day], index));
  }
