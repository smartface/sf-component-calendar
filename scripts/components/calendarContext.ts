...
function removeSelection(context, state) {
  ...
  context
    .find(state.selectedDay, {
      removeClassName: raiseTargetNotfound(state.selectedDay)
    })
  ...
  switch (action.type) {
    case INIT_CONTEXT_ACTION_TYPE:
      const reducer = (acc: string[], actor: Actor, name: string) => {
        if (name.indexOf('_weekDay') > 0) acc.push(name);
        return acc;
      };
    ...
    case 'resetDays':
      ...
      (newState.tomonthTargets || []).forEach((element) => {
        actor = context.find(element as any, {
          pushClassNames: raiseTargetNotfound(target)
        }) as Stylable;
    ...
    case 'daySelected':
      ...
      if (newState.selectedDay) {
        actor = context.find(newState.selectedDay as any, {
          pushClassNames: raiseTargetNotfound(target)
        }) as Stylable;
    ...
    case 'tomonth':
      ...
      if (!newState.tomonthTargets) newState.tomonthTargets = [];
    ...
    case 'changeCalendar':
      const mapFn = (actor: Stylable) => {
        if (!actor || actor.getName() === undefined) raiseTargetNotfound(target);

        const className = actor.getInitialClassName();
        actor.resetClassNames([...className, '#' + actor.getName(), '#' + actor.getName() + '-lang--' + action.lang, '#' + actor.getName() + '-os--' + System.OS]);
