...
export function getDatePos(
  ...
  const monthPos = (date.month === month.date.month && 'current') || (date.month === month.nextMonth.date.month && 'next') || (date.month === month.previousMonth.date.month && 'prev');
  ...
  switch (monthPos) {
    ...
    case 'next':
      const posNext = calculateDatePosinNext(month.startDayOfMonth, month.daysCount, date.day);
